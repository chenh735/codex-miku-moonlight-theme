...
在窗口顶部、Windows 最小化按钮左侧点击圆形 `✦` 按钮打开主题设置。任务页透明度可在 5%–100% 之间调节，默认值为 30%；100% 会完全移除任务页白色玻璃底层，背景最清晰，但可能降低长文本和代码的可读性。星光、月光呼吸、城市灯光、流光边框和流星均可独立启停，也可以暂停全部动态。
...
对话页使用可读性玻璃层，背景穿透强度由任务页透明度控制。点击窗口顶部、Windows 最小化按钮左侧的圆形 `✦`，将透明度调整到合适数值；当前默认是 30%。
